...
export const createRow = (
  model: Readonly<RowModel>,
  source: Readonly<TreeState>,
  onChangeCb: (value: Readonly<TreeState>) => void
): Readonly<Row> => {
  let rowAPI: RowAPI = {
    toggleChildren: () => {
      const nextState: Readonly<TreeState> = TreeState.toggleChildren(
        source,
        model
      );
    ...
    updateData: (newData: RowData) => {
      const nextState: Readonly<TreeState> = TreeState.updateData(
        source,
        model,
        newData
      );
